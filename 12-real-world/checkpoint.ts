 ...
 *
 * 1. SettingsSchema: a zod object — apiUrl (string, OPTIONAL in the JSON
 *    itself, since it can come from the env instead), retries (a
 *    non-negative integer), featureFlags (array of strings).
 * 2. Settings: the fully-resolved shape the rest of the app uses —
 *    apiUrl, retries, featureFlags, all REQUIRED (apiUrl gets filled in
 *    from the env when the JSON doesn't have it).
 * 3. loadSettings(text, env = process.env):
...
// TODO: apiUrl optional string, retries non-negative int, featureFlags string[].
export const SettingsSchema = z.unknown()

// TODO: { apiUrl: string; retries: number; featureFlags: string[] }
export type Settings = unknown

// TODO: (text: string, env?: Record<string, string | undefined>) => Result<Settings, string>
